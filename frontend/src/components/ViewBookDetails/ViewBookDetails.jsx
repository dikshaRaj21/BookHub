...
const ViewBookDetails = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await axios.get(
        `https://bookheaven-mu3j.onrender.com/api/v1/get-book-by-id/${id}`
      );
  ...
  const handleFavourite = async () =>{
    const response = await axios.put("https://bookheaven-mu3j.onrender.com/api/v1/add-book-to-favourite", {}, {headers})
    alert(response.data.message)
  }

  const handleCart = async () =>{
    const response = await axios.put("https://bookheaven-mu3j.onrender.com/api/v1/add-to-cart", {}, {headers})
    alert(response.data.message)
  }


  const deleteBook = async () =>{
    const response = await axios.delete("https://bookheaven-mu3j.onrender.com/api/v1/delete-book", {headers})
    alert(response.data.message)
    navigate("/all-books")
  }
    ...
    <>
      {Data && (
        <div className="px-4 md:px-12 py-8 bg-zinc-900 flex flex-col lg:flex-row gap-8">
          ...
          {" "}
            <div className="flex flex-col lg:flex-row p-12 justify-around bg-zinc-800 rounded">
              {" "}
              <img
                src={Data.url}
              ...
              {isLoggedIn === true && role === "admin" && (
                ...
                <button className="text-red rounded lg:rounded-full text-3xl p-3 mt-8 md:mt-0 lg:mt-8 bg-white flex items-center justify-center"
                onClick={deleteBook}
                >
